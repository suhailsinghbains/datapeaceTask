...
export default class Table extends Component {
    ...
    componentDidMount() {
        ...
        this.TableRender(JSON.parse(result));
    ...
    SortForLoop(Col) {
        for (let j = 1; j < document.getElementById("TableHere").rows.length - 1; j++) {
            this.SortAlgo(Col, j);
        }
    }
    SortAlgo(Col, k) {
        let TableData = document.getElementById("TableHere");
        let min, pos;
        min = TableData.rows.item(k-1).cells[Col].innerHTML;
        for (let i = k; i < (TableData.rows.length - 1); i++) {

            if (min > TableData.rows.item(i + 1).cells[Col].innerHTML) {
                min = TableData.rows.item(i + 1).cells[Col].innerHTML;
                pos = i + 1;
                let j = i + 1;
            }
        }
        let temp = TableData.rows.item(pos);
        TableData.removeChild(TableData.rows.item(pos));
        TableData.prepend(temp);
    }
    ...
    TableRender(items) {
        var Display = []; //For Keeping the Table Relevant
        var LastIndex = 5;
        let j = 0;
        this.Properties = [
            "first_name",
        ...
        this.Properties.forEach((item) => {
            ...
            Heading.addEventListener('click', function (item) {
                that.SortForLoop(that.Properties.indexOf(this.innerHTML));
            });
            TempTable.appendChild(Heading);
        });
        Table.appendChild(TempTable);
        this.ReturnTableDataTag(items).forEach((item) => {
